...
fn s1() -> u32 {
    ...
    let mut p_iter = Box::new(p.into_iter()) as Box<dyn Iterator<Item = (Vec<u32>, Vec<u32>)>>;
    for _ in 0..9 {
        p_iter = Box::new(p_iter.flat_map(|(drawn, to_draw)| draw_digit(drawn, to_draw)));
    }
...
fn draw_digit(drawn: Vec<u32>, to_draw: Vec<u32>) -> impl Iterator<Item = (Vec<u32>, Vec<u32>)> {
