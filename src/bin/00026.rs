...
fn reciprocal(divisor: u16) -> Option<(String, usize)> {
    ...
    vec_r[0] = 1;
    hash.insert(1, 0);
    for i in 1..MAX { // preventing from overflow
...
fn div(dividend: u16, divisor: u16) -> (String, u16) {
    ...
    if dividend*10/divisor >= 1 {
        return (format!("{}", dividend*10/divisor), dividend*10%divisor);
    }

    if dividend*100/divisor >= 1 {
        return (format!("0{}", dividend*100/divisor), dividend*100%divisor);
    }

    if dividend*1000/divisor >= 1 {
        return (format!("00{}", dividend*1000/divisor), dividend*1000%divisor);
    }
